/**
 * Blake3 Deno Benchmark
 * Run with: deno run --allow-read deno-bench.ts
 */
...
function processChunk(input: Uint8Array, chunkStart: number, chunkLen: number, chunkCounter: number, flags: number): Uint32Array {
...
function mergeParents(left: Uint32Array, right: Uint32Array, flags: number, isRoot: boolean): Uint32Array {
...
function hashJS(input: Uint8Array, outputLen = 32): Uint8Array {
  const flags = 0;
...
console.log('Blake3 Deno Benchmark');
console.log('=====================\n');

console.log(`Deno version: ${Deno.version.deno}`);
console.log(`V8 version: ${Deno.version.v8}`);
console.log(`WASM SIMD supported: ${checkWasmSimd() ? 'YES' : 'NO'}`);
...
for (const [len, expected] of Object.entries(EXPECTED)) {
  ...
  if (got === expected) {
    ...
    failed++;
    console.log(`✗ len=${len}: got ${got.slice(0, 16)}... expected ${expected.slice(0, 16)}...`);
  }
}
...
console.log('---------------------');
console.log('Size (bytes) | Pure JS');
console.log('-------------|------------');
...
for (const size of sizes) {
  ...
  console.log(`${size.toString().padStart(12)} | ${throughput.toFixed(1).padStart(8)} MB/s`);
}

console.log('\n');
console.log('Summary:');
console.log('- Pure JS uses SMI variables, inlined permutation, LE fast path');
console.log('- WASM SIMD compress4x would add ~1.4x on large inputs');
console.log('- Workers would add parallelism for very large inputs');
