...
// Detect little-endian for fast path
const IS_LE = new Uint8Array(new Uint32Array([1]).buffer)[0] === 1;

// Reusable buffer for block reading
const blockBuf = new Uint32Array(16);

/**
 * Blake3 compress function - the core of the algorithm
 * Uses SMI variables for V8 optimization
 */
function compress(
  cv: Uint32Array,
  m: Uint32Array,
  ...
  // State as SMI variables (V8 optimization)
  ...
  let s12 = counter | 0, s13 = (counter / 0x100000000) | 0;
  let s14 = blockLen | 0, s15 = flags | 0;

  // Message words as variables
  ...
  for (let i = 0; i < 7; ++i) {
    ...
    // Message permutation (inlined, no temp array)
    if (i < 6) {
      const t0 = m0, t1 = m1;
      m0 = m2; m2 = m3; m3 = m10; m10 = m12; m12 = m9; m9 = m11; m11 = m5; m5 = t0;
      m1 = m6; m6 = m4; m4 = m7; m7 = m13; m13 = m14; m14 = m15; m15 = m8; m8 = t1;
    }
  }
...
/** Read a block into Uint32Array with padding */
function readBlock(
  input: Uint8Array,
  offset: number,
  length: number,
...
/** Fast path for aligned little-endian reads */
function readBlockLE(input: Uint8Array, offset: number, out: Uint32Array): boolean {
...
/** Convert CV to output bytes */
function cvToBytes(cv: Uint32Array, len: number): Uint8Array {
  const result = new Uint8Array(len);
  for (let i = 0; i < Math.min(8, Math.ceil(len / 4)); i++) {
    const w = cv[i];
    const b = i * 4;
    if (b < len) result[b] = w & 0xff;
    if (b + 1 < len) result[b + 1] = (w >> 8) & 0xff;
    if (b + 2 < len) result[b + 2] = (w >> 16) & 0xff;
    if (b + 3 < len) result[b + 3] = (w >> 24) & 0xff;
  }
  return result;
}
 ...
 * ```
 */
export function blake3(input: Uint8Array | string, outputLength: number = 32): Uint8Array {
  ...
  // Single chunk case (≤1024 bytes) - inline for speed
  ...
  for (let c = 0; c < numChunks; c++) {
    ...
    // Merge complete subtrees (but not on last chunk)
    if (c < numChunks - 1) {
      let totalChunks = c + 1;
 ...
 */
blake3.hex = function(input: Uint8Array | string, outputLength: number = 32): string {
 ...
 */
export function keyedHash(key: Uint8Array, input: Uint8Array | string): Uint8Array {
  if (key.length !== 32) {
    throw new Error('Key must be exactly 32 bytes');
  }
  // TODO: Implement keyed mode
